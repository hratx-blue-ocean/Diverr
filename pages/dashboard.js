...
export async function getServerSideProps(context) {
  ...
  const resultUserTags = await fetch(
    `${process.env.NEXTAUTH_URL}/api/user/${email}/tags`
  );
  ...
  const resultUserLogs = await fetch(
    `${process.env.NEXTAUTH_URL}/api/user/${email}/logs`
  );
